...
def test_enviar_mensagens_agendadas(mock_logger, mock_all_artistas, mock_filter):
    ...
    mock_message.sent = False
    mock_message.deve_enviar.return_value = True  # Mensagem deve ser enviada
    mock_message.enviar.return_value = None  # Método de envio simulado
    ...
    # Verificando se o log de erro não foi chamado
...
def test_verificar_status(mock_logger):
    # Criando artistas no banco de dados de teste
    artista1 = Artista.objects.create(nome="Artista 1", telefone="123456789", email="teste1@email.com", cpf="123.456.789-01")
    artista2 = Artista.objects.create(nome="Artista 2", telefone=None, email="teste2@email.com", cpf="987.654.321-00")  # Incompleto

    # Criando mensagens associadas aos artistas
    message1 = Message.objects.create(artista=artista1, conteudo="Mensagem 1", send_date=now(), sent=False)
    message2 = Message.objects.create(artista=artista2, conteudo="Mensagem 2", send_date=now(), sent=False)

    # Mockando métodos específicos
    with patch.object(Artista, 'deve_ser_atualizado', side_effect=[False, True]) as mock_deve_ser_atualizado, \
         patch.object(Artista, 'atualizar_status') as mock_atualizar_status, \
         patch.object(Message, 'deve_ser_atualizado', return_value=True) as mock_message_deve_atualizar, \
         patch.object(Message, 'atualizar_status') as mock_message_atualizar_status:
        
        # Chamando a função de verificação de status
        verificar_status()

        # Verificando chamadas em artistas
        assert mock_deve_ser_atualizado.call_count == 2
        mock_deve_ser_atualizado.assert_any_call()
        mock_atualizar_status.assert_called_once_with()

        # Verificando chamadas em mensagens
        assert mock_message_deve_atualizar.call_count == 2
        assert mock_message_atualizar_status.call_count == 2

        # Verificando logs
        mock_logger.info.assert_any_call(f"Status do artista {artista2.id} atualizado.")
        mock_logger.info.assert_any_call(f"Status da mensagem {message1.id} atualizado.")
        mock_logger.info.assert_any_call(f"Status da mensagem {message2.id} atualizado.")     
